...
while True:
    ...
    def rotate(origin, point, angle):
        ox, oy = origin
        px, py = point
        qx = int(math.floor(ox + math.cos(angle / x) *
                            (px - ox) - math.sin(angle) * (py - oy)))
        qy = int(math.floor(oy + math.sin(angle) *
                            (px - ox) + math.cos(angle) * (py - oy)))
        return qx, qy
    ...
    rotation_matrix1_inv = cv2.getRotationMatrix2D(
        (cols / 2, rows / 2), (-angle1), 1)
    rotation_matrix2 = cv2.getRotationMatrix2D((cols / 2, rows / 2), angle2, 1)
    rotation_matrix2_inv = cv2.getRotationMatrix2D(
        (cols / 2, rows / 2), (-angle2), 1)
    ...
    if len(faces) == 0:
        ...
        for (x, y, w, h) in faces:
            for i in range(
                ...
                for (x, y, w, h) in faces:
					# 255 adalah index warna putih
                    output_ori[y:y + h, x:x + w] = [255]
	...
	# hal ini menjaga agar proses tetap dilakukan pada resolusi tertinggi
	# yang dimungkinkan kamera
    output_ori_show = cv2.resize(output_ori, size_im)
	...
	# konvert ke gray
    gray_output_ori_roted = cv2.cvtColor(
        output_ori_roted, cv2.COLOR_BGR2GRAY)
