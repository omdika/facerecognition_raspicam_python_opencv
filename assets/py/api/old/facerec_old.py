...
webcam = cv2.VideoCapture('http://192.168.2.189:8081/frame.mjpg')
global framecount
global framerate
framecount = 0
framerate = 10
    ...
    while(not rval):# Check if this is the frame closest to 5 seconds
        if framecount == round((framerate * 5),0):
            ...
            for i in range(len(faces)):
                ...
                #print st
                send_data = {"user_id_s":"01","channel_s":"7","device_id_s":"01","name_s":detect_name,"event_dt":st}
                json_data = json.dumps(send_data)
                #publish.single(topic="demomars", payload=json_data, hostname="192.168.2.130")
                print json_data
