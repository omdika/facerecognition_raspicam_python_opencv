...
# create pid file
pid = str(os.getpid())
f = open('/var/www/html/setopbox3/assets/py/log/facerec_pid2', 'w+')
f.write(pid)
f.close()
...
LOG_FILENAME = '/var/www/html/setopbox3/assets/py/log/push.log.out2'
...
(images, lables, names, id, idusers, genders, ages) = ([], [], {}, 0, [], [], [])
...
for (subdirs, dirs, files) in os.walk(fn_dir):
    ...
    for subdir in dirs:
        ...
        for row in results:
            ...
            age = row[3]
            if name == subdir:
				#print "name is " +name+ " subdir " +subdir
				gender.append()
				"""  
        ...
        for filename in os.listdir(subjectpath):
            ...
            for row in results:
                ...
                if name == subdir:
				    ...
				    (iduser, name, gender, age )
				    genders.append(gender)
				    ages.append(age)
				    idusers.append(iduser)
				    images.append(cv2.imread(path, 0))
				    lables.append(int(lable))
				    print "appending "+str(lable)
        print "subdirs end"
        id += 1
    ...
    while(not rval):# Check if this is the frame closest to 5 seconds
        if framecount == round((framerate * 5),0):
            ...
            for i in range(len(faces)):
                ...
                detect_gender = genders[prediction[0]]
                detect_age = ages[prediction[0]]
                detect_idusers = idusers[prediction[0]]
                ...
                for row in results:
                    ...
                    if name == detect_name:
                        #print st"""
                send_data = {"userid_i":detect_idusers,"channel_s":"7","age_i":detect_age, "gender_s":detect_gender,"name_s":detect_name,"event_s":st}
                json_data = json.dumps(send_data)
                print json_data
                logging.debug(json_data)
                #publish.single(topic="demomars02", payload=json_data, hostname="192.168.2.3")
