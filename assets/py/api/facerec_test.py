...
def pushData(data):
    ...
    for row in results:
        ...
        if name == data:
            ...
            #publish.single("demomars02", json_data, hostname="192.168.1.10")
...
while True:
    ...
    face_cascade = cv2.CascadeClassifier('/var/www/html/setopbox3/assets/py/haarcascade_frontalface_default.xml')
    ...
    try:
        os.remove('/var/www/html/setopbox3/assets/py/api/Test.png')
        print "old image removed "
        logging.debug("old image removed "+st)
    except:
        print "no image"
        logging.debug("no old image foound "+st)
    ...
    camera.capture('/var/www/html/setopbox3/assets/py/api/Test.png')
    logging.debug("new image captured "+st)
    ...
    img = cv2.imread('/var/www/html/setopbox3/assets/py/api/Test.png')
    img_pre = img.copy()
    ...
    if len(faces) == 0:
        ...
        print "Undetected Faces on Original Position "+st
        logging.debug("Undetected Faces on Original Position "+st)
        end = time.time()
        elapseTime(end)

    else:
        print "Found Faces on Original Position " +st
        ...
        for (x, y, w, h) in faces:
            for i in range(
                ...
                print "Prediction value is " + '%s' % (prediction[1]) + " " +st
                ...
                for n in range(20):
                    ...
                    if prediction[1] >= thr:
                        ...
                        logging.debug("Found Faces on Original Position "+st)
    ...
    if len(faces_rot1) == 0:
        print "Undetected Faces on Original Position 1 "+st
        logging.debug("Undetected Faces on Original Position 1 "+st)
        end = time.time()
        elapseTime(end)

    else:
        print "Found Faces on Rotation 1 "+st
            ...
            for i in range(len(faces_rot1)):
                ...
                for m in range(20):
                    if prediction[1] >= thr:
                        ...
                        logging.debug("Found Faces on Rotation 1 "+st)
    ...
    if len(faces_rot2) == 0:
        ...
        print "Undetected Faces on Original Position 2 "+st
        logging.debug("Undetected Faces on Original Position 2 "+st)
        end = time.time()
        elapseTime(end)
        ...
        for (x2, y2, w, h) in faces_rot2:
            ...
            for i in range(len(faces_rot2)):
                ...
                print "Prediction Value:" + '%s' % (prediction[1]) + " " +st
                for m in range(20):
                    if prediction[1] >= thr:
                        print names_rot2[prediction[0]]
                        logging.debug("Found Faces on Rotation 2 "+st)
